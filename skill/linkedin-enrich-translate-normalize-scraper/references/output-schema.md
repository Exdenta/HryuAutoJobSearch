# LinkedIn normalized output schema

Actor: `nomad-agent/linkedin-enrich-translate-normalize-scraper`

Contract version: `nomad-agent-job-v1`
...
- Common typed objects are strict: do not silently add, discard, or rename fields within v1.
- Static card, detail-page, and JobPosting JSON-LD values win. Optional LLM enrichment can fill only still-null allowed paths.
...
| `identity` | object | Stable source identity. |
| `data` | object | Normalized facts. |
| `custom` | null | LinkedIn has no v1 source extension. |
| `llm` | object | Optional enrichment status and provenance. |
| `raw` | object | Complete plain-text and HTML source descriptions. |

## Identity and company

| Path | Type |
|---|---|
| `identity.source` | string; always `linkedin` |
...
| `data.domains` | array of strings or null |
| `data.domainsRaw` | array of strings or null |
...
| Path | Type |
|---|---|
| `data.seniority.raw` | array of strings or null |
| `data.seniority.levels` | array of `intern`, `entry`, `junior`, `mid`, `senior`, `lead`, `principal`, `manager`, `director`, `executive`, `R1`, `R2`, `R3`, or `R4`; or null |
...
Only `completed` enrichment may have filled paths. Static values and

"""Lossless convenience parser for normalized EURAXESS Actor dataset rows."""
...
def _nullable_strings(value: object, path: str) -> tuple[str, ...] | None:
    if value is None:
        return None
    if not isinstance(value, list):
        raise OutputParseError(f"{path} must be an array or null")
    parsed: list[str] = []
    for index, element in enumerate(value):
        parsed_value = _nullable_string(element, f"{path}[{index}]")
        if parsed_value is None:
            raise OutputParseError(f"{path}[{index}] must not be null")
...
class EuraxessJob:
    ...
    academic_levels: tuple[str, ...] | None
    research_infrastructure_staff_position: str | None
    llm_status: str
    normalized: Mapping[str, Any]


def parse_euraxess_output(item: Mapping[str, Any]) -> EuraxessJob:
    """Validate one EURAXESS row's stable envelope and return a lossless view."""
    ...
    if custom.get("schemaId") != CUSTOM_SCHEMA_ID:
        raise OutputParseError(
            f"custom.schemaId must be {CUSTOM_SCHEMA_ID!r}"
        )
    ...
    return EuraxessJob(
        ...
        deadline=_nullable_string(
            application.get("deadline"), "data.application.deadline"
        ),
        ...
        academic_levels=_nullable_strings(
            custom_data.get("academicLevelRaw"),
            "custom.data.academicLevelRaw",
        ),
        research_infrastructure_staff_position=_nullable_string(
            custom_data.get("researchInfrastructureStaffPosition"),
            "custom.data.researchInfrastructureStaffPosition",
        ),
